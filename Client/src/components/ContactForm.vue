<template>
    <Form @submit="submitContact" :validation-schema="contactFormSchema">
        ...
        <div class="form-group">
            ...
            <select name="groupId" class="form-control" v-model="contactLocal.groupId">
                <option value="null">Chọn...</option>
                <option v-for="Group of Groups" :value="Group._id" :key="Group._id">{{ Group.name }}</option>
            </select>
        </div>
...
export default {
    ...
    methods: {
        submitContact() { this.$emit("submit:contact", this.contactLocal) },
        deleteContact() { this.$emit("delete:contact", this.contactLocal.id) },
        async getAllGroups() {
            try {
